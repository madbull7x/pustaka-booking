    ...
    { 
        ...
        foreach ($kategori as $k) { 
            ...
            if ($this->form_validation->run() == false) { 
                ...
                //membatasi jenis image yang bisa diupload $pisah = explode('.', $filename); 
                //memisahkan nama file dengan extention $namaimage = 'img' . time() . in_array($pisah[1], $ext_list); 
                //menghasilkan nama image baru move_uploaded_file($_FILES['image']['tmp_name'], base_url('./assets/img/upload') . $namaimage); unlink('assets/img/upload/' . $this->input->post('old_pict', TRUE));*/
